  ...
  class ReplProcessor < Byebug::CommandProcessor
      ...
      allowing_other_threads do
        @session.lock do
          ...
          unless @session.should_stop?(@context.frame.file)
            ...
            if @repeated_flow > REPEATED_FLOW_COUNT_THRESHOLD && Time.now.to_f - @last_repeat_at > REPEATED_FLOW_TIME_THRESHOLD
              @previous_flow = Mole::ControlFlow.new(:step_out)
            end
            @previous_flow ||= Mole::ControlFlow.new(:next)
    ...
    def handle_frame_command(options)
      ...
      if next_frame.nil?
        ...
        # @screen_manager.console.puts 'Error: Frame not found. There should be an error with Mole.'
    ...
    def handle_switch_filter_command(_options = {})
      @config.filter = Mole::PathFilter.next_filter(@config.filter)
