...
module Mole
  ...
  class PathFilter
    ...
    def self.next_filter(current)
      index = Mole::PathFilter::FILTERS.index(current) || -1
      index = (index + 1) % Mole::PathFilter::FILTERS.length
      Mole::PathFilter::FILTERS[index]
    end

    def initialize(config: nil, path_classifier: nil)
      @config = config || Mole.config
      @path_classifier = path_classifier || Mole::PathClassifier.new
    ...
    def match_everything?(path)
      ...
      case type
      when Mole::PathClassifier::TYPE_SOURCE_TREE, Mole::PathClassifier::TYPE_UNKNOWN
        !match_excluded?(path)
      when Mole::PathClassifier::TYPE_GEM
        !match_excluded?(info[0], expand_path: false) && !match_excluded?(path)
      when Mole::PathClassifier::TYPE_STDLIB
        !match_excluded?(info[0], expand_path: false) && !match_excluded?(path)
      else
        true
      end
    ...
    def match_gems?(path)
      ...
      when Mole::PathClassifier::TYPE_RUBY_SCRIPT, Mole::PathClassifier::TYPE_EVALUATION
        true
      when Mole::PathClassifier::TYPE_INTERNAL
        false
      end
    end
    ...
    def match_application?(path)
      ...
      when Mole::PathClassifier::TYPE_SOURCE_TREE, Mole::PathClassifier::TYPE_UNKNOWN
        !match_excluded?(path)
      when Mole::PathClassifier::TYPE_GEM, Mole::PathClassifier::TYPE_STDLIB
        match_included?(info[0], expand_path: false) || match_included?(path)
      when Mole::PathClassifier::TYPE_RUBY_SCRIPT
        true
      when Mole::PathClassifier::TYPE_EVALUATION
        false
      when Mole::PathClassifier::TYPE_INTERNAL
        false
      end
    ...
    def match_source_tree?(path)
      ...
      case type
      when Mole::PathClassifier::TYPE_SOURCE_TREE
        !match_excluded?(path)
      when Mole::PathClassifier::TYPE_UNKNOWN
        match_included?(path)
      when Mole::PathClassifier::TYPE_GEM, Mole::PathClassifier::TYPE_STDLIB
        match_included?(info[0], expand_path: false) || match_included?(path)
      when Mole::PathClassifier::TYPE_RUBY_SCRIPT
