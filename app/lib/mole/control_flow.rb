...
module Mole
  ...
  class ControlFlow
    THROW_KEYWORD = :mole_control_flow
    ALLOW_LIST = {
    ...
    class << self
      def dispatch(command, arguments = {})
        if command.is_a?(Mole::ControlFlow)
      ...
      def listen
        ...
        if flow.nil? || flow.is_a?(Mole::ControlFlow)
          flow
        else
          raise Mole::Error, 'Control flow misused!'
        end
      end
    end
    ...
    def validate!
      if command.to_s.empty?
        raise Mole::Error, 'Control command is empty'
      end
